...
- **若 `targets/<env>.md` 不存在、或里面没有本次要测的站**（常见于刚建的产品仓库）：**停下来按上面的表格式和用户一起补**——问清环境名、站 `id`（短横线小写、跨环境同名）、URL、说明，写进 `targets/<env>.md` 再开工。**绝不自己编 URL**。插件里 `templates/targets.example.md` 是带注释的起点。

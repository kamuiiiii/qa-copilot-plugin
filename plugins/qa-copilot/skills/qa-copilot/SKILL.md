...
这个 skill 只读写五类文件，全在当前产品仓库内。先认清各自的角色、读 / 写方向、以及**权威说明在哪一节**——下文凡提到这些文件的散落处，细节都收口到这里指向的章节，不重复展开。

| 文件 / 目录                | 角色                                                                     | 读 / 写                                    | 权威说明（见下文章节） |
| -------------------------- | ------------------------------------------------------------------------ | ------------------------------------------ | ---------------------- |
| `targets/<env>.md`         | **输入**：每个环境一组被测站清单                                         | 读（首次跑由 bootstrap 写）                | 「输入：环境与站」     |
| `secrets/cookies-all.json` | **输入**：全量登录态 cookie（所有站 / 环境共一份）                       | 读（首次跑由 bootstrap 导出）              | 「浏览器接入」         |
| `runs/<run-id>/`           | **产物**：每次测试一个独立目录（`ac.md` / `screenshots/` / `report.md`） | 写                                         | 「产物」               |
| `notes/<site>.md`          | **跨 run 复用**：每站的导航 / 定位地图（可选）                           | 读（step 2）+ 写（step 6）                 | 「产品备忘 `notes/`」  |
| `specs/<flow>.md`          | **跨 run 复用**：可复用的正式 AC（测试用例母本，按流程切，可选）          | 读（step 2 回归直跑 / 当草案）+ 写（step 6 晋升） | 「AC 规范库 `specs/`」 |

两条贯穿全篇、最容易踩错的纪律先记牢，后面只在专节展开：

- **runs 彼此隔离**：每个 run 从零起、不读旧 run、`report.md` 不写跨 run 引用。 → 「run 之间：隔离与沉淀」
- **跨 run 复用只有两类资产**：`notes/` 记「路」（怎么进、怎么点），`specs/` 记「测什么」（可复用 AC）。 → 「产品备忘 `notes/`」「AC 规范库 `specs/`」

> `.gitignore` 须忽略 `secrets/` 与 `runs/`（`notes/` / `specs/` / `targets/` 入库），缺则补上；首次跑由 bootstrap 一并处理。
...
- **若 `targets/<env>.md` 不存在、或里面没有本次要测的站**（常见于刚建的产品仓库 / 新环境）：**停下来补，绝不自己编 URL**。空仓库 / 新环境首次跑走 `reference/bootstrap.md` 的完整初始化引导（建 target → 登录态 → cookie）；运行中途才发现缺站，就和用户一起按上面的表格式补——问清环境名、站 `id`（短横线小写、跨环境同名）、URL、说明，写进 `targets/<env>.md` 再继续。

## 工作流总览

每次测试分七步走，前一步不完成不进入下一步。**每个 run 独立、从零起**：新 session 不继承上一次的任何记忆，**绝不因为磁盘上存在上一轮的 `ac.md` / `report.md` 就跳过下面的步骤直接执行**——每次都重新解析意图、起草 AC、等用户确认，哪怕这次的请求和上次字面完全相同。（这条管的是**旧 `runs/` 的产物**：它们是某一次的快照、未经策展，不可当先验。**`specs/` 是正当例外**——用户点名要跑的正式 AC 可走 step 2 回归模式跳过勘探与重起草，细节与"为何不矛盾"见「AC 规范库 specs/」。旧 run 的读取隔离见「run 之间：隔离与沉淀」。）
...
2. **确定本次 AC** — 这一步有两种入口，按用户意图二选一：

   **A. 回归模式（用户点名要跑 `specs/` 里已有的 spec）** — 例如"跑一下 checkout 这个 spec"、"按下单回归用例测一轮"，或意图明确命中某个现成 `specs/<flow>.md`。**不勘探、不重新起草**：spec 里的 AC 已是成品测试用例。直接读出来作为本次 AC，向用户做一次**轻量 go/no-go**（"按 `specs/checkout.md` 这 N 条 AC 跑、涉及 @order 站，开跑？"——让用户能拦住选错 / 已作废的 spec），确认后进 step 3 snapshot、step 4 执行。漂移不靠前置勘探防、靠执行兜底——完整机制见「AC 规范库 specs/」。

   **B. 起草模式（新测 / 探索 / 没有命中的 spec）** — 走下面完整的勘探 + 起草 + 确认门。若有**部分相关**的 spec，按勘探纪律把它当**草案种子**读进来提速，但仍要勘探核对、起草、等确认。

   下面是 **B 模式**的完整流程——把意图翻译成一份可验证的 **验收标准（AC, Acceptance Criteria）** 列表，每条是一个能独立判定通过 / 失败的布尔判断。**每条 AC 必须用 Gherkin Step 规范表达**，至少包含 `Given` / `When` / `Then` 三类步骤，同类步骤的并列 / 反向条件用 `And` / `But` 追加：
   ...
   **不要指望用户能把前置条件写全**——用户是判定者，不一定清楚某个动作背后要先满足什么（人擅长"看到具体东西判断对错"，不擅长"凭空回忆要写什么"）。所以前置链的责任在你：**起草前先做一次有界勘探**，把探到的前置写进 `Given` 草案让用户来认 / 改，而不是让用户从零想清楚。
   ...
   - **先看 AC 规范库（有就读）**：产品仓库若有 `specs/<flow>.md` **部分相关**（不是完全命中——完全命中且用户要回归就走上面的 A 模式，不进这套勘探），读进来作为 **AC 草案的种子**省掉从零起草。但它只是**母本不是结论**：里面的 `Given` 前置链仍要靠本次勘探现场核对，改完仍走确认门。详见 `### AC 规范库 specs/`。
...
3. **写 AC 清单** — 把用户确认后的 AC 列表写入 `ac.md`，格式同上。**这是本次实际被测的那一版、随 run 永久冻结**——即便它派生自 `specs/` 母本，也是独立快照，不是指向母本的引用（母本后续会演进，证据不能跟着变）。若派生自规范库，在 `ac.md` 顶部记一行血缘戳便于溯源：`# 派生自 specs/<flow>.md @ <YYYY-MM-DD>`。
...
6. **回填产品备忘 + 晋升 AC 规范** — 两件对称的沉淀：
   - **notes**：本 run 勘探 / 执行中**确认为稳定**的入口、前置链、定位策略、怪异行为，按站回写进 `notes/<site>.md`（带 run-id 戳，格式见 `### 产品备忘 notes/`）；qa / uat 行为不一致的写进该站的「环境差异」小节。发现与既有条目冲突就地改正。没有可沉淀的稳定结构就跳过。
   - **specs**：两种情形回写——① **新测 / 草案模式**下，本 run 的 AC 跑下来**验证有效、值得复用**，把它去实例化后**晋升**进 `specs/<flow>.md`；② **回归模式**下执行撞到漂移、与用户确认了 AC 的新写法，就**更新**既有 spec（母本只留可复用的测试定义、不带本次具体值，格式见 `### AC 规范库 specs/`）。一次性的、与本次特殊上下文强绑定的 AC 不晋升。是否晋升 / 更新由你判断，拿不准就问用户。
7. **关闭浏览器实例**

## run 之间：隔离与沉淀

> 承上「每个 run 独立、从零起」。本节讲一个 run 与别的 run 的关系：什么时候续写 / 开新 run、run 之间如何隔离（不串数据）、以及允许跨 run 复用的两类精选资产——`notes/` 导航地图与 `specs/` AC 母本。

### 隔离：不读旧 run、不写跨 run 引用

- **绝不读取旧 run，不要把旧 run 的发现当"先验知识"**。陷阱（比如某个 UI 控件点了会跳页、某个 toast 用 emotion 随机 class 渲染）每次都应该正常踩、正常记录、正常写进 Open question —— 它们是产品当下真实的样子，规避它们等于帮产品掩盖问题。
- **例外，除非用户明确提及**：用户明确说"参考上一轮"、"在 run X 基础上补测"、"对比 run A 和 run B"，或当前 run 的 `ac.md` 自身写明要参照另一个 run。
- **跨 run 复用只走两类资产**（见下文与 `### AC 规范库 specs/`）：`notes/` 沉淀**地图不是发现**（只喂勘探与定位、不喂判定 / 观察值）；`specs/` 沉淀**可复用的正式 AC**（喂测试定义、不喂结论；用户点名要跑就直接当本次 AC，只是参考就当草案）。两者都不是"读旧 run"——原始旧 run 仍隔离不读。

### 产品备忘 `notes/`（跨 run 的导航 / 定位地图）

产品仓库里一个**可选**目录 `notes/`，**每个站一个文件 `notes/<site>.md`**。只回答两个问题——**怎么进到这个流程（入口）、怎么点到关键元素（语义定位）**，外加已知的坑。给下个 run 的勘探与定位提速，省掉从零摸路、重摸 selector。
...
**用前必验、失配就改**：每条都现场验证后再用，别当事实（notes 是线索不是结论）。对不上（入口没了、钮改了、点名的实例找不到）→ **最多降级再试一次**（换个同类有效项、或从零摸一次定位），还不行就**停下报告 + 改这条**——改正本身往往就是一条 finding（产品变了）。绝不因为"notes 这么写"反复硬找。
...
### AC 规范库 `specs/`（跨 run 的可复用测试定义）

产品仓库里一个**可选**目录 `specs/`，**按流程 / feature 组织 `specs/<flow>.md`**（如 `specs/checkout.md`、`specs/hdr-copy.md`）。它存的是**精选、可复用的正式 AC——测试用例本身**（不是草稿），格式与 `ac.md` 一致（Gherkin），但**不绑任何 run、不带本次具体值**。

**两种消费方式，对应 step 2 的两种入口：**

- **回归模式（用户点名要跑某个 spec）**：spec 里就是成品 AC，**不再勘探、不再重新起草**——直接读出来当本次 AC，轻量 go/no-go 后 snapshot 进 `ac.md` 执行。漂移靠**执行兜底**：跑的时候撞到 `Given` / `When` / `Then` 与现场不一致，就停下来问用户（见「撞到未知前置」），别硬跑也别擅自改。
- **草案种子（部分相关、或新测时顺手参考）**：当成 step 2 的草案起点提速，仍要勘探核对、起草、过确认门。

这两种方式不冲突：**spec 是否权威由"用户是不是点名要跑它"决定**——点名跑就是回归（信它、执行兜底），只是参考就是种子（验它、确认门）。

> **specs 不按站切、不按环境切——这点和 `notes/` 正相反**：notes 是某个站的导航地图，天然属于站；specs 是测试场景，天然属于**流程**。一个流程可能横跨多个站（跨站联动），硬塞进某一个站的目录会割裂——用 AC 内部的 `@site` 标注表达涉及哪些站（对接「跨站测试」那套 `@site` 维度）即可。同一个站在 qa / uat 下的 AC 本就该复用，所以也**不按环境分文件**：qa / uat 共用一份母本，真有环境差异写进 AC 的 `Given` 或加 `### 环境差异` 小节内联。

**先认清它和另外两类文件的本质区别——这是整套复用设计的关键，搞混就会破坏证据：**

|                          | 它是什么                          | 生命周期                              | 谁来用                      |
| ------------------------ | --------------------------------- | ------------------------------------- | --------------------------- |
| `specs/<flow>.md`        | 可复用的正式 AC（母本，按流程切）  | **可变**，随产品演进、会被晋升 / 改写 | step 2 回归直跑 / 当草案；step 6 晋升 |
| `runs/<id>/ac.md`        | 本次实际确认并测的那一版（快照）  | **不可变**，随 run 永久冻结           | 本 run 的执行与 report 复述 |
| `notes/<site>.md`        | 导航 / 定位地图（怎么走、怎么点） | 可变，跨 run 攒                       | step 2 提速勘探与定位       |

**specs 与 ac.md 不是重复，是相反生命周期**：母本会演进，证据要冻结。所以**无论哪种模式，本次实际跑的那一版都要 snapshot 进 `runs/<id>/ac.md`**（带血缘戳）——回归模式是把 spec 原样拷一份，草案模式是把确认后那版拷一份。母本日后再怎么改，旧 run 的 ac.md / 截图 / report 复述都不受影响、永远自洽。这正是为什么 spec 不能"只活在 specs/、run 里存个指针"。

**写什么——记「可复用的测试意图」，不记「本次的具体料」**（与 notes「记路不记料」同源）：

- **记**：每条 AC 的 Gherkin 结构（`Given` 前置链的**结构** / `When` 动作 / `Then` 观察对象 + 预期），以及对应的截图清单结构。
- **不记**：本次的具体实例与值（餐厅名 / slug / 记录 id、`Showing 60 results` 的 60、某账户状态）、本次的判定结论 / 观察值、run-id 绑定。`Then` 的预期落到**稳定的观察对象 + 关系**（如"状态栏文案随勾选数递增"），不写死成某次的具体数字。

**漂移兜底——产品变了怎么发现**：两种模式抓漂移的时机不同，但结论一样（撞到不一致 → 停下报告 + 据此在 step 6 更新母本，**改动本身往往就是一条 finding**）：

- **回归模式**靠**执行期**抓：不前置勘探，跑到哪条 AC 与现场对不上（入口没了 / `Given` 不成立 / `Then` 观察对象变了），就地停下问用户，别硬跑、别擅自改 AC 蒙混过去。
- **草案模式**靠**勘探期**抓：起草前现场核对每条 `Given` 前置链，对不上当场改草案，再过确认门。

**回归模式可跳过勘探与重新起草，但跳不过两件事：① 本次 AC 仍要 snapshot 进 `ac.md`；② 执行撞到不一致仍要停下来问。** 它与「工作流总览」line 63"绝不因磁盘上有旧 ac.md 就跳步"不矛盾——那条管的是**未经策展的旧 `runs/` 产物**，specs 是用户点名、被策展的正式用例，是其正当例外。

**组织**：按 `<flow>` 分文件（一个流程 / feature 一份，跨站流程仍是一份、内部用 `@site` 标注），不按站、不按环境拆（理由见本节开头的说明框）。`specs/` 与各文件**首次有内容才创建**（不预建空目录）。每条 / 每份带晋升来源的 run-id 戳便于溯源。
...
```
runs/<run-id>/
├── ac.md                  — 用户确认后的 AC 列表 + 截图清单（本次冻结快照；派生自 specs 时顶部带血缘戳）
├── screenshots/
...
**回归模式（直跑 spec、没经过前置勘探）尤其靠这条兜底**：spec 是按上次的产品状态写的，这次可能已漂移。跑到某条 AC 的 `Given` 不再成立、入口没了、`Then` 的观察对象变了，就在这里停下问用户——**别硬跑，更别擅自把 AC 改得能过**。确认新写法后据此在 step 6 更新 spec（这次漂移本身就是一条 finding）。

### AC 是执行边界，不是起点——不自行加码

**确认后的 AC 就是这次测试的完整边界：写了的逐条做完，没写的一律不做。** 执行期最常见的偏离不是漏测，而是**自作主张多做**——模型会把"多覆盖一点"误当成尽责。它不是尽责，是偏离：多出来的东西不在用户认可的范围里，是噪声不是证据，还拖慢用户等待、可能污染后端数据。

把"想多做"的冲动一律视作**信号**：说明你觉得 AC 不够。那是 **AC 阶段**该提的事——停下来告诉用户、补进 AC、等确认，**不在执行期自己补**。常见的几种加码，全部不许：

- **扩样本量**：AC 说"一条 / 任一行 / 某个详情"就**只测一条**——"任意 / 任一"约束的是选择自由（哪条都行），不是数量；随手定一条有效的，定了不再换、不再加。要测多条 / 全集合必须 AC 自己写明（见下表「集合断言」档）。
- **加测相邻功能**：顺手点开别的 tab、捎带验证个排序 / 筛选、试试别的入口——AC 没写就不碰。
- **自加测试维度**：AC 没要求就别自己补边界值、异常输入、健壮性、性能观察。
- **顺藤摸瓜**：执行中看到可疑的点，**记进 report 的 Open question**，不要当场展开深挖、改变本次测试的范围。

一句话自检：**每个动作都要能指回某条 AC 的某个 `When`**。指不回去，就是越界——停，别做。
...
| **集合断言**            | "列表 / dropdown 必须包含 N 类元素"、"所有状态都能作为 source"、"全字段保留"（**AC 显式断言的是集合属性才算**；"某条详情正常显示"是单条断言，N=1，不进此档、不要 dump 多条） | 单张截图说明不了"集合属性"，必须配一份 `eval` dump 出全集合的 textContent / 属性数组（如 `Array.from(document.querySelectorAll('...')).map(e=>e.textContent.trim())`），把结果写进 `report.md` 对应 AC 的"观察"里。AC 判定基于 dump 文本里能逐项点出的存在性，**不基于"截图里看到的几条"**。                                                                                                                                   |
...
```bash
browser-use --session $SID eval "<触发动作的 click>" \
  && browser-use --session $SID screenshot screenshots/N0-after-confirm-0s.png \
  && sleep 1 && browser-use --session $SID screenshot screenshots/N1-after-confirm-1s.png \
  && sleep 1 && browser-use --session $SID screenshot screenshots/N2-after-confirm-2s.png \
  && sleep 1 && browser-use --session $SID screenshot screenshots/N3-after-confirm-3s.png
```
...
`browser-use --session $SID --json state` 已经能给你按钮文案、`disabled` 标志、横幅文本、表格行、toast 文案、对话框结构。"动作生效了吗"这类问题 90% 都能用纯文本回答。
...
默认流程：`browser-use --session $SID screenshot <path>` → 记日志 → 继续。不要 `Read`。
...
`browser-use --session $SID state` 给的 element index 在每次重渲染后都会变。为了找回同一个按钮再 dump 一次 state 是纯浪费。优先级（按本项目实测稳定度排）：

1. **`eval` 内 querySelector + textContent 匹配**（最稳）：
   ```bash
   browser-use --session $SID eval "Array.from(document.querySelectorAll('button')).find(b => b.textContent.trim() === 'Submit').click()"
   ```
   能精确定位、不被 row-click handler 之类外层冒泡干扰。
2. **CSS selector via eval**：`browser-use --session $SID eval "document.querySelector('.copy-btn').click()"`。当目标有稳定 class / id 时用。
3. **state 里的 index**：`browser-use --session $SID click 8263`。只有当上面两种都不行（如 antd `<label>` 包 `<input>` 的特殊结构、视口外元素、需要 CDP 在元素中心精确点击的场景）时才用；记得 idx **每次重渲染就过期**，不要跨 dump 复用。

**注意 / 反模式**：`browser-use --session $SID click "Submit"` 这种"传文本作为 click 子命令参数"的写法**当前 CLI 不支持**（`click` 只接受 `<index>` 或 `<x> <y>` 坐标，会报 `invalid int value`）。要按文本点击就走优先级 #1 的 `eval` 路径。
...
`browser-use` 的动作本身会阻塞到返回。每次点击后塞个 `sleep 0.3–1.5s` 是 cargo cult。只有在确实在和某个异步事件竞争时才等待——而且要用 `browser-use --session $SID wait` 加一个具体 selector，而不是盲目 sleep。

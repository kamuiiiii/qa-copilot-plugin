...
这个 skill 只读写四类文件，全在当前产品仓库内。先认清各自的角色、读 / 写方向、以及**权威说明在哪一节**——下文凡提到这些文件的散落处，细节都收口到这里指向的章节，不重复展开。
...
- **notes 是唯一跨 run 复用物**：只记「路」（怎么进、怎么点）不记「料」（看到什么、选了哪个），且用前必验。 → 「产品备忘 `notes/`」

> `.gitignore` 须忽略 `secrets/` 与 `runs/`（`notes/` 与 `targets/` 入库），缺则补上；首次跑由 bootstrap 一并处理。
...
每次测试分七步走，前一步不完成不进入下一步。**每个 run 独立、从零起**：新 session 不继承上一次的任何记忆，**绝不因为磁盘上存在上一轮的 `ac.md` / `report.md` 就跳过下面的步骤直接执行**——每次都重新解析意图、起草 AC、等用户确认，哪怕这次的请求和上次字面完全相同。（旧 run 数据的读取隔离见下「run 之间：隔离与沉淀」一节；唯一能跨 run 复用的是 `notes/` 导航地图。）
...
> 承上「每个 run 独立、从零起」。本节讲一个 run 与别的 run 的关系：什么时候续写 / 开新 run、run 之间如何隔离（不串数据）、以及**唯一**允许跨 run 复用的东西——`notes/` 导航地图。
...
- **唯一的跨 run 复用是 `notes/` 地图**（见下一小节）：它沉淀的是**地图不是发现**，只喂勘探与定位、不喂判定 / 观察值，所以不破此纪律——原始旧 run 仍隔离不读。

...
`connect` 需要用户的 Chrome 开了远程调试。有两条路，**任选其一**即可。如果 `connect` 失败，告诉用户：

> 没有检测到开了远程调试的 Chrome。两种方式选一种——
>
> **方式 A：用命令行参数重启 Chrome**（推荐，无需手动开关）
>
> 先关掉当前 Chrome，然后用这个命令重新启动：
>
> ```
> open -a "Google Chrome" --args --remote-debugging-port=9222
> ```
>
> **方式 B：在已开着的 Chrome 里打开开关**（不用重启）
>
> 地址栏访问 `chrome://inspect/#remote-debugging`，把页面里的远程调试开关打开。
>
> 任一方式弄好后告诉我，我重试 connect。

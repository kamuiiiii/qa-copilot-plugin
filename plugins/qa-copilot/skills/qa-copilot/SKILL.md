...
2. **起草 AC 并等用户确认（强制）** — 把意图翻译成一份可验证的 **验收标准（AC, Acceptance Criteria）** 列表，每条是一个能独立判定通过 / 失败的布尔判断。**每条 AC 必须用 Gherkin Step 规范表达**，至少包含 `Given` / `When` / `Then` 三类步骤，同类步骤的并列 / 反向条件用 `And` / `But` 追加：
   ...
   **不要指望用户能把前置条件写全**——用户是判定者，不一定清楚某个动作背后要先满足什么（人擅长"看到具体东西判断对错"，不擅长"凭空回忆要写什么"）。所以前置链的责任在你：**起草前先做一次有界勘探**，把探到的前置写进 `Given` 草案让用户来认 / 改，而不是让用户从零想清楚。
   ...
   - **先读产品备忘（有就读）**：产品仓库若有 `notes/<site>.md`，对本 run 每个站读一遍，把入口 / 前置链 / 定位策略 / 怪异行为当勘探起点——"先去这儿看 / 这样点得到"。但 notes 只**给勘探提速、不免掉勘探**，且是**线索不是结论**仍要现场验证（详见 `### 产品备忘 notes/`）；尤其里面的**具体实例**（餐厅名 / slug / 地址 / id）只是上次样本，现场**重新选一个有效的**——**找不到它是正常的，不是要你反复探索硬找它**。
...
### 产品备忘 `notes/`（跨 run 的导航 / 定位地图）

产品仓库里一个**可选**目录 `notes/`，**每个站一个文件 `notes/<site>.md`**。只回答两个问题——**怎么进到这个流程（入口）、怎么点到关键元素（语义定位）**，外加已知的坑。给下个 run 的勘探与定位提速，省掉从零摸路、重摸 selector。它和上面「跨 run 引用纪律」不冲突：沉淀的是**地图不是发现**，地图只喂勘探与定位，发现（判定 / 观察值）永远现场抓、永远进 report。

**一条准则定写什么——记「路」（怎么走、怎么点），不记「料」（看到什么、值多少、这次选了哪个）。** "路"跨 run 不变、值得攒；"料"是发现，现场抓、进 report。别再纠结某条"稳不稳"——按这条分就行，写错了下个 run 一用就暴露（见下「用前必验」）。

- **记**：入口路径、前置链的**结构**（要先满足哪几步）、**语义**定位策略（textContent 匹配、稳定 class / id）、稳定的怪异行为（点 X 会同时触发 Y）。
- **不记**：具体实例（餐厅名 / slug / 记录 id / 地址）、坐标 / element index / emotion 随机 class、条数 / 账户状态 / 某次的值、判定结论 / 观察值。
- 最易写错的一处：前置链里"要选一个实例"的步骤，**只记怎么选、不记选了哪个**——写"随便选一家 HDR、列表第一家即可"，绝不写上次那个名字 / id。具体实例下个 run 就没了，害你死磕一个不存在的东西反复探索。

**用前必验、失配就改**：每条都现场验证后再用，别当事实。对不上（入口没了、钮改了、点名的实例找不到）→ **最多降级再试一次**（换个同类有效项、或从零摸一次定位），还不行就**停下报告 + 改这条**——改正本身往往就是一条 finding（产品变了）。绝不因为"notes 这么写"反复硬找。

**组织**：每站 **≤6 条 bullet、粗颗粒**（写到要拆小步 / 贴参数 / 列分支，就是在抄流程而非画地图，停下删）；**按站拆不按环境拆**（URL 是 `targets/` 的事，地图跨环境基本不变，qa / uat 共用一份，真有差异就在该文件加 `### 环境差异` 小节内联，戳标明哪个环境）；跨站联动链放**发起操作那个站**的文件。每条带 run-id 确认戳，`notes/` 与各站文件**首次有内容才创建**（不预建空目录）。格式示例（`notes/order.md`，注意全篇 ≤6 条、零具体实例、零坐标）：

```markdown
## @order 下单流程

- 入口：顶部 "Order" → 餐厅列表页 `/order`
- 前置链：先选一家餐厅（随便哪家 HDR 都行，不选则菜品列表为空）→ 加购使购物车非空 → 进结账页（配送时间 / 小费 / 支付方式通常已默认就绪）
- 定位：加购钮用 `button` 里 textContent 以 "Add" 开头的那个；下单钮用 textContent 匹配 "Place delivery order"；购物车按钮是 header 里的图标钮，用稳定 aria-label / 语义结构定位，**别用坐标**
- 怪异：首次进站有 OneTrust cookie 弹窗，点 "Allow All" 关掉再操作
...
已经解析出来的 selector 要缓存，整个会话复用。**跨 run 复用**的只能是优先级 #1 / #2 那种**策略级语义定位**（textContent 匹配、稳定 class / id）——把它回写进 `notes/<site>.md` 的「定位」条目（见 `### 产品备忘 notes/`），下个 run 直接拿来用。优先级 #3 的 element index、以及任何坐标 / 几何定位都是 session 级 / 易碎、重渲染或改尺寸就过期，**绝不**回写 notes。
